...
library(classdata)
library(ggplot2)
max(ames$`YearBuild `, na.rm = TRUE)
min(ames$`YearBuild `, na.rm = TRUE)
ggplot(data = ames, aes(x = factor(YearBuilt)))  + geom_bar() + labs(x = "Year Built")
...
ggplot(data = ames, aes(x = 'YearBuilt', y = `Sale Price`)) +
  geom_point() +
  labs(x = 'Year Built', y = 'Sale Price') +  coord_flip()
